...
noinline void gpu_sched_switch(void *data, const char *gpu_name, u64 timestamp,
...
noinline void gpu_job_enqueue(void *data, u32 ctx_id, u32 job_id, const char *type)
{
...
#endif
void pm_qos_update_request(void *data, int pm_qos_class, s32 value)
{
	char class_name[64];

	MET_TRACE("pm_qos_class=%s value=%d\n",
...
static void met_event_start(void)
{
	...
	do {
		...
		if (met_tracepoint_probe_reg("gpu_sched_switch", gpu_sched_switch))
			break;

		if (met_tracepoint_probe_reg("gpu_job_enqueue", gpu_job_enqueue))
			break;
