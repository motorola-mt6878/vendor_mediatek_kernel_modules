...
#include "gpueb_met_ipi_handle.h" /* for met_ipi_to_gpueb_command */


/*****************************************************************************
 * struct & enum declaration
 *****************************************************************************/
struct gpueb_met_event_header {
 ...
 *****************************************************************************/
static void ondiemet_gpueb_start(void);
static void ondiemet_gpueb_stop(void);
static int ondiemet_gpueb_print_help(char *buf, int len);
static int ondiemet_gpueb_process_argument(const char *arg, int len);
static int ondiemet_gpueb_print_header(char *buf, int len);
...
static struct gpueb_met_event_header met_event_header[MAX_MET_RTS_EVENT_NUM];
static char gpueb_help[] = "  --gpueb_common=rts_event_name\n";
static char header[] = 	"met-info [000] 0.0: gpueb_common_header: ";
static bool met_event_header_updated = false;
static int nr_dts_header = 0;

struct metdevice met_gpueb_common = {
	.name = "gpueb_common",
	.owner = THIS_MODULE,
	.type = MET_TYPE_BUS,
	.cpu_related = 0,
	.ondiemet_mode = 1,
	.ondiemet_start = ondiemet_gpueb_start,
	.ondiemet_stop = ondiemet_gpueb_stop,
	.ondiemet_process_argument = ondiemet_gpueb_process_argument,
	.ondiemet_print_help = ondiemet_gpueb_print_help,
	.ondiemet_print_header = ondiemet_gpueb_print_header,
};
...
static int ondiemet_gpueb_print_help(char *buf, int len)
{
	return snprintf(buf, PAGE_SIZE, gpueb_help);
}


static int get_rts_header_from_dts_table(struct gpueb_met_event_header* __met_event_header)
{
	int idx;
	...
	/*get rts root node*/
	np = of_find_node_by_name(NULL, "gpueb-rts-header");
	if (!np) {
		pr_debug("unable to find gpueb-rts-header\n");
		return 0;
	}
...
static int ondiemet_gpueb_print_header(char *buf, int len)
{
	...
	len = 0;
	met_gpueb_common.header_read_again = 0;
	if (is_dump_header == 0) {
	...
	for (i = read_idx; i < nr_dts_header; i++) {
		if (met_event_header[i].chart_line_name) {
			...
			if ((len + write_len) < PAGE_SIZE) {
				...
				met_gpueb_common.header_read_again = 1;
				read_idx = i;
				return len;
			}
		}
...
static void ondiemet_gpueb_start(void)
{
	...
	ondiemet_module[ONDIEMET_GPUEB] |= ID_COMMON;
}
...
static void update_event_id_flag(int event_id)
{
	...
	ipi_buf[3] = 0;
	res = met_ipi_to_gpueb_command((void *)ipi_buf, 0, &rdata, 1);

	met_gpueb_common.mode = 1;
}
...
EXPORT_SYMBOL(met_gpueb_common);
