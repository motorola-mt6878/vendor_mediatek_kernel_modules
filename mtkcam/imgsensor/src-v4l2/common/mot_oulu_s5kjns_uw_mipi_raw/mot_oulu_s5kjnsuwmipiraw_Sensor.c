...
static struct subdrv_mode_struct mode_struct[] = {
	{
		...
		.mode_setting_table = addr_data_pair_preview_mot_oulu_s5kjns_uw,
		.mode_setting_len = ARRAY_SIZE(addr_data_pair_preview_mot_oulu_s5kjns_uw),
...
const struct subdrv_entry mot_oulu_s5kjns_uw_mipi_raw_entry = {
	.name = "mot_oulu_s5kjns_uw_mipi_raw",
	.id = MOT_OULU_S5KJNS_UW_SENSOR_ID,
	.pw_seq = pw_seq,
	...
	while (ctx->s_ctx.i2c_addr_table[i] != 0xFF) {
		...
		do {
			*sensor_id = (subdrv_i2c_rd_u8(ctx, addr_h) << 8) |
			...
			//sensor id + 3 for s5kjns_uw on oulu
			if(*sensor_id == 0x38EE)
			*sensor_id += 3;
...
{
	DRV_LOG(ctx, "E\n");
	DRV_LOG(ctx, "MOT OULU S5KJNS_UW init start\n");
	...
	i2c_table_write(ctx, addr_data_pair_init_mot_oulu_s5kjns_uw,
			ARRAY_SIZE(addr_data_pair_init_mot_oulu_s5kjns_uw));
	DRV_LOG(ctx, "MOT OULU S5KJNS_UW init end\n");
	DRV_LOG(ctx, "X\n");
}
