...
static int s5kjns_uw_crc_reverse_byte(int data)
{
	return ((data * 0x0802LU & 0x22110LU) |
		(data * 0x8020LU & 0x88440LU)) * 0x10101LU >> 16;
}

static int32_t s5kjns_uw_check_crc16(struct subdrv_ctx *ctx, uint8_t  *data, uint32_t size, uint32_t ref_crc)
{
	int32_t crc_match = 0;
	uint16_t crc = 0x0000;
	...
	for (i = 0; i < size; i++) {
		tmp_reverse = s5kjns_uw_crc_reverse_byte(data[i]);
		tmp = data[i] & 0xff;
		for (j = 0; j < 8; j++) {
	...
	crc_reverse = (s5kjns_uw_crc_reverse_byte(crc_reverse) << 8) |
		s5kjns_uw_crc_reverse_byte(crc_reverse >> 8);
...
static int mot_s5kjns_uw_read_hw_ggc_data(struct subdrv_ctx *ctx)
{
	int ret = 0;
	u16 ref_crc = 0;
	...
	if (s5kjns_uw_check_crc16(ctx, s5kjns_hw_ggc_data, S5KJNS_HW_GGC_DATA_LEN, ref_crc)) {
...
static void mot_s5kjns_uw_apply_hw_ggc_data(void *sensor_ctx)
{
	struct subdrv_ctx *ctx = (struct subdrv_ctx *)sensor_ctx;

	if (!s5kjns_hw_ggc_data_ready) {
		mot_s5kjns_uw_read_hw_ggc_data(ctx);
	}
...
{
	mot_s5kjns_uw_apply_hw_ggc_data(sensor_ctx);
}
