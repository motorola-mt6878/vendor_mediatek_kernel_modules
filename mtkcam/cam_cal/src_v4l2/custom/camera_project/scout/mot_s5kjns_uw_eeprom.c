...
unsigned int s5kjns_uw_mot_do_factory_verify(struct EEPROM_DRV_FD_DATA *pdata, unsigned int *pGetSensorCalData);
static unsigned int s5kjns_uw_mot_do_2a_gain(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);


#define S5KJNS_UW_MOT_EEPROM_ADDR 0x00
#define S5KJNS_UW_MOT_EEPROM_DATA_SIZE 0x1B4E
#define S5KJNS_UW_MOT_SERIAL_NUMBER_ADDR 0x15
#define S5KJNS_UW_MOT_MNF_ADDR 0x00
#define S5KJNS_UW_MOT_MNF_DATA_SIZE 37
#define S5KJNS_UW_MOT_MNF_CHECKSUM_ADDR 0x25
#define S5KJNS_UW_MOT_AF_ADDR 0x27
#define S5KJNS_UW_MOT_AF_DATA_SIZE 24
#define S5KJNS_UW_MOT_AF_CHECKSUM_ADDR 0x3F
#define S5KJNS_UW_MOT_AWB_ADDR 0x41
#define S5KJNS_UW_MOT_AWB_DATA_SIZE 43
#define S5KJNS_UW_MOT_AWB_CHECKSUM_ADDR 0x6C
#define S5KJNS_UW_MOT_LSC_ADDR 0x0C96
#define S5KJNS_UW_MOT_LSC_DATA_SIZE 0x074C
#define S5KJNS_UW_MOT_LSC_CHECKSUM_ADDR 0X13E2
#define S5KJNS_UW_MOT_PDAF1_ADDR 0x13E4
#define S5KJNS_UW_MOT_PDAF2_ADDR 0x15D4
#define S5KJNS_UW_MOT_PDAF_DATA1_SIZE 496
#define S5KJNS_UW_MOT_PDAF_DATA2_SIZE 1004
#define S5KJNS_UW_MOT_PDAF_DATA_SIZE (S5KJNS_UW_MOT_PDAF_DATA1_SIZE+S5KJNS_UW_MOT_PDAF_DATA2_SIZE)
#define S5KJNS_UW_MOT_PDAF_CHECKSUM_ADDR 0x19C0
#define S5KJNS_UW_MOT_AF_SYNC_DATA_ADDR 0x19C4
#define S5KJNS_UW_MOT_AF_SYNC_DATA_SIZE 24
#define S5KJNS_UW_MOT_AF_SYNC_DATA_CHECKSUM_ADDR 0x19DC

#define S5KJNS_UW_MOT_MTK_NECESSARY_DATA_ADDR 0x19DE
#define S5KJNS_UW_MOT_MTK_NECESSARY_DATA_SIZE 18
#define S5KJNS_UW_MOT_MTK_NECESSARY_DATA_CHECKSUM_ADDR 0x19F0
...
static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x00000003, 0x32443832, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		...
		{0x00000001, S5KJNS_UW_MOT_LSC_ADDR, S5KJNS_UW_MOT_LSC_DATA_SIZE, mot_do_single_lsc},
		{0x00000001, 0x00000027, 0x00000045, s5kjns_uw_mot_do_2a_gain},
		{0x00000001, S5KJNS_UW_MOT_PDAF1_ADDR, S5KJNS_UW_MOT_PDAF_DATA_SIZE, mot_do_pdaf},
		...
		{0x00000001, S5KJNS_UW_MOT_MNF_ADDR, S5KJNS_UW_MOT_MNF_DATA_SIZE, mot_do_manufacture_info}
	}
};

struct STRUCT_CAM_CAL_CONFIG_STRUCT mot_s5kjns_uw_eeprom = {
	.name = "mot_s5kjns_uw_eeprom",
	.sensor_type = UW_CAMERA,
	.check_layout_function = mot_layout_no_ck,
	.read_function = Common_read_region,
	.mot_do_factory_verify_function = s5kjns_uw_mot_do_factory_verify,
	.layout = &cal_layout_table,
	.sensor_id = MOT_SCOUT_S5KJNS_UW_SENSOR_ID,
	.i2c_write_id = 0xA0,
	.max_size = 0x8000,
	.serial_number_bit = 16,
	.enable_preload = 1,
	.preload_size = S5KJNS_UW_MOT_EEPROM_DATA_SIZE,
};
		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	int af_addr = S5KJNS_UW_MOT_AF_ADDR;
	int af_size = S5KJNS_UW_MOT_AF_DATA_SIZE;
	int awb_addr = S5KJNS_UW_MOT_AWB_ADDR;
	int awb_size = S5KJNS_UW_MOT_AWB_DATA_SIZE;
	...
	if(pCamCalData->sensorID == MOT_SCOUT_S5KJNS_UW_SENSOR_ID) {
		//af posture calibration data
		unsigned char AF_POSTURE[S5KJNS_UW_MOT_MTK_NECESSARY_DATA_SIZE+2];
		unsigned int af_posture_data_offset = S5KJNS_UW_MOT_MTK_NECESSARY_DATA_ADDR;
		...
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				af_posture_data_offset, S5KJNS_UW_MOT_MTK_NECESSARY_DATA_SIZE+2, (unsigned char *) AF_POSTURE);
		if (read_data_size > 0)
			err = CAM_CAL_ERR_NO_ERR;
		else {
			pCamCalData->Single2A.S2aBitEn = CAM_CAL_NONE_BITEN;
		...
		checkSum = AF_POSTURE[S5KJNS_UW_MOT_MTK_NECESSARY_DATA_SIZE]<<8 | AF_POSTURE[S5KJNS_UW_MOT_MTK_NECESSARY_DATA_SIZE+1];
		if(check_crc16(AF_POSTURE, S5KJNS_UW_MOT_MTK_NECESSARY_DATA_SIZE, checkSum)) {
...
unsigned int s5kjns_uw_mot_do_factory_verify(struct EEPROM_DRV_FD_DATA *pdata, unsigned int *pGetSensorCalData)
{
	...
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			S5KJNS_UW_MOT_EEPROM_ADDR, S5KJNS_UW_MOT_EEPROM_DATA_SIZE, (unsigned char *)pCamCalData->DumpAllEepromData);
	if (read_data_size <= 0) {
		return CAM_CAL_ERR_NO_DEVICE;
	}
	//mnf check
	checkSum = (pCamCalData->DumpAllEepromData[S5KJNS_UW_MOT_MNF_CHECKSUM_ADDR])<< 8
		|(pCamCalData->DumpAllEepromData[S5KJNS_UW_MOT_MNF_CHECKSUM_ADDR+1]);
	debug_log("checkSum  = 0x%x", checkSum);

	if(check_crc16(pCamCalData->DumpAllEepromData, S5KJNS_UW_MOT_MNF_DATA_SIZE, checkSum)) {
	...
	memcpy(pCamCalData->serial_number, &pCamCalData->DumpAllEepromData[S5KJNS_UW_MOT_SERIAL_NUMBER_ADDR],
		pCamCalData->serial_number_bit);

	//af check
	checkSum = (pCamCalData->DumpAllEepromData[S5KJNS_UW_MOT_AF_CHECKSUM_ADDR])<< 8
		|(pCamCalData->DumpAllEepromData[S5KJNS_UW_MOT_AF_CHECKSUM_ADDR+1]);
	debug_log("checkSum  = 0x%x", checkSum);

	if(check_crc16(pCamCalData->DumpAllEepromData+S5KJNS_UW_MOT_AF_ADDR, S5KJNS_UW_MOT_AF_DATA_SIZE, checkSum)) {
	...
	//awb check
	if((mot_check_awb_data(pCamCalData->DumpAllEepromData+S5KJNS_UW_MOT_AWB_ADDR, S5KJNS_UW_MOT_AWB_DATA_SIZE + 2)) == NO_ERRORS) {
	...
	//lsc check
	checkSum = (pCamCalData->DumpAllEepromData[S5KJNS_UW_MOT_LSC_CHECKSUM_ADDR])<< 8
		|(pCamCalData->DumpAllEepromData[S5KJNS_UW_MOT_LSC_CHECKSUM_ADDR+1]);
	debug_log("checkSum  = 0x%x", checkSum);

	if(check_crc16(pCamCalData->DumpAllEepromData+S5KJNS_UW_MOT_LSC_ADDR, S5KJNS_UW_MOT_LSC_DATA_SIZE, checkSum)) {
	...
	//pdaf check
	checkSum = (pCamCalData->DumpAllEepromData[S5KJNS_UW_MOT_PDAF_CHECKSUM_ADDR])<< 8
		|(pCamCalData->DumpAllEepromData[S5KJNS_UW_MOT_PDAF_CHECKSUM_ADDR+1]);
	checkSum1 = (pCamCalData->DumpAllEepromData[S5KJNS_UW_MOT_PDAF_CHECKSUM_ADDR+2])<< 8
		|(pCamCalData->DumpAllEepromData[S5KJNS_UW_MOT_PDAF_CHECKSUM_ADDR+3]);
	debug_log("checkSum  = 0x%x, checkSum1 = 0x%x", checkSum, checkSum1);

	if(check_crc16(pCamCalData->DumpAllEepromData+S5KJNS_UW_MOT_PDAF1_ADDR, S5KJNS_UW_MOT_PDAF_DATA1_SIZE, checkSum)
		&& check_crc16(pCamCalData->DumpAllEepromData+S5KJNS_UW_MOT_PDAF2_ADDR, S5KJNS_UW_MOT_PDAF_DATA2_SIZE, checkSum1)) {
	...
	//af sync data check
	checkSum = (pCamCalData->DumpAllEepromData[S5KJNS_UW_MOT_AF_SYNC_DATA_CHECKSUM_ADDR])<< 8
		|(pCamCalData->DumpAllEepromData[S5KJNS_UW_MOT_AF_SYNC_DATA_CHECKSUM_ADDR+1]);
	debug_log("checkSum  = 0x%x", checkSum);
	if(check_crc16(pCamCalData->DumpAllEepromData+S5KJNS_UW_MOT_AF_SYNC_DATA_ADDR, S5KJNS_UW_MOT_AF_SYNC_DATA_SIZE, checkSum)) {
	...
	checkSum = (pCamCalData->DumpAllEepromData[S5KJNS_UW_MOT_MTK_NECESSARY_DATA_CHECKSUM_ADDR])<< 8
		|(pCamCalData->DumpAllEepromData[S5KJNS_UW_MOT_MTK_NECESSARY_DATA_CHECKSUM_ADDR+1]);
	debug_log("checkSum  = 0x%x", checkSum);
	if(check_crc16(pCamCalData->DumpAllEepromData+S5KJNS_UW_MOT_MTK_NECESSARY_DATA_ADDR, S5KJNS_UW_MOT_MTK_NECESSARY_DATA_SIZE, checkSum)) {
