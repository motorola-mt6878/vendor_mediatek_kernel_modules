...
static unsigned int mot_gc13a2_vcm_blad_do_factory_verify(struct EEPROM_DRV_FD_DATA *pdata, unsigned int *pGetSensorCalData);
static unsigned int mot_gc13a2_vcm_blad_do_2a_gain(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);

#define MOT_GC13A2_VCM_BLAD_EEPROM_ADDR 0x00
#define MOT_GC13A2_VCM_BLAD_EEPROM_DATA_SIZE 0x192A
#define MOT_GC13A2_VCM_BLAD_SERIAL_NUMBER_ADDR 0x15
#define MOT_GC13A2_VCM_BLAD_MNF_ADDR 0x00
#define MOT_GC13A2_VCM_BLAD_MNF_DATA_SIZE 37
#define MOT_GC13A2_VCM_BLAD_MNF_CHECKSUM_ADDR 0x25
#define MOT_GC13A2_VCM_BLAD_AF_ADDR 0x27
#define MOT_GC13A2_VCM_BLAD_AF_DATA_SIZE 24
#define MOT_GC13A2_VCM_BLAD_AF_CHECKSUM_ADDR 0x3F
#define MOT_GC13A2_VCM_BLAD_AWB_ADDR 0x41
#define MOT_GC13A2_VCM_BLAD_AWB_DATA_SIZE 43
#define MOT_GC13A2_VCM_BLAD_AWB_CHECKSUM_ADDR 0x6C
#define MOT_GC13A2_VCM_BLAD_LSC_ADDR 0x0BCE
#define MOT_GC13A2_VCM_BLAD_LSC_DATA_SIZE 1868
#define MOT_GC13A2_VCM_BLAD_LSC_CHECKSUM_ADDR 0X131A
#define MOT_GC13A2_VCM_BLAD_PDAF1_ADDR 0x131C
#define MOT_GC13A2_VCM_BLAD_PDAF2_ADDR 0x150C
#define MOT_GC13A2_VCM_BLAD_PDAF_DATA1_SIZE 496
#define MOT_GC13A2_VCM_BLAD_PDAF_DATA2_SIZE 1004
#define MOT_GC13A2_VCM_BLAD_PDAF_DATA_SIZE (MOT_GC13A2_VCM_BLAD_PDAF_DATA1_SIZE+MOT_GC13A2_VCM_BLAD_PDAF_DATA2_SIZE)
#define MOT_GC13A2_VCM_BLAD_PDAF_CHECKSUM_ADDR 0x18F8
#define MOT_GC13A2_VCM_BLAD_AF_SYNC_DATA_ADDR 0x18FC
#define MOT_GC13A2_VCM_BLAD_AF_SYNC_DATA_SIZE 24
#define MOT_GC13A2_VCM_BLAD_AF_SYNC_DATA_CHECKSUM_ADDR 0x1914

#define MOT_GC13A2_VCM_BLAD_MTK_NECESSARY_DATA_ADDR 0x1916
#define MOT_GC13A2_VCM_BLAD_MTK_NECESSARY_DATA_SIZE 18
#define MOT_GC13A2_VCM_BLAD_MTK_NECESSARY_DATA_CHECKSUM_ADDR 0x1928
...
static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x00000003, 0x32443832, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		...
		{0x00000001, MOT_GC13A2_VCM_BLAD_LSC_ADDR, MOT_GC13A2_VCM_BLAD_LSC_DATA_SIZE, mot_do_single_lsc},
		{0x00000001, 0x00000027, 0x00000045, mot_gc13a2_vcm_blad_do_2a_gain},
		{0x00000001, MOT_GC13A2_VCM_BLAD_PDAF1_ADDR, MOT_GC13A2_VCM_BLAD_PDAF_DATA_SIZE, mot_do_pdaf},
		...
		{0x00000001, MOT_GC13A2_VCM_BLAD_MNF_ADDR, MOT_GC13A2_VCM_BLAD_MNF_DATA_SIZE, mot_do_manufacture_info}
	}
};

struct STRUCT_CAM_CAL_CONFIG_STRUCT mot_gc13a2_vcm_blad_eeprom = {
	.name = "mot_gc13a2_eeprom",
	.sensor_type = UW_CAMERA,
	.check_layout_function = mot_layout_no_ck,
	.read_function = Common_read_region,
	.mot_do_factory_verify_function = mot_gc13a2_vcm_blad_do_factory_verify,
	...
	.enable_preload = 1,
	.preload_size = MOT_GC13A2_VCM_BLAD_EEPROM_DATA_SIZE,
};
		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	int af_addr = MOT_GC13A2_VCM_BLAD_AF_ADDR;
	int af_size = MOT_GC13A2_VCM_BLAD_AF_DATA_SIZE;
	int awb_addr = MOT_GC13A2_VCM_BLAD_AWB_ADDR;
	int awb_size = MOT_GC13A2_VCM_BLAD_AWB_DATA_SIZE;
	...
	if(pCamCalData->sensorID == MOT_VIENNA_GC13A2_VCM_BLAD_SENSOR_ID) {
		//af posture calibration data
		unsigned char AF_POSTURE[MOT_GC13A2_VCM_BLAD_MTK_NECESSARY_DATA_SIZE+2];
		unsigned int af_posture_data_offset = MOT_GC13A2_VCM_BLAD_MTK_NECESSARY_DATA_ADDR;
		...
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				af_posture_data_offset, MOT_GC13A2_VCM_BLAD_MTK_NECESSARY_DATA_SIZE+2, (unsigned char *) AF_POSTURE);
		if (read_data_size > 0)
			err = CAM_CAL_ERR_NO_ERR;
		else {
			pCamCalData->Single2A.S2aBitEn = CAM_CAL_NONE_BITEN;
		...
		checkSum = AF_POSTURE[MOT_GC13A2_VCM_BLAD_MTK_NECESSARY_DATA_SIZE]<<8 | AF_POSTURE[MOT_GC13A2_VCM_BLAD_MTK_NECESSARY_DATA_SIZE+1];
		if(check_crc16(AF_POSTURE, MOT_GC13A2_VCM_BLAD_MTK_NECESSARY_DATA_SIZE, checkSum)) {
			debug_log("check_crc16 ok");
			err = CAM_CAL_ERR_NO_ERR;
		} else {
			debug_log("check_crc16 err");
...
static unsigned int mot_gc13a2_vcm_blad_do_factory_verify(struct EEPROM_DRV_FD_DATA *pdata, unsigned int *pGetSensorCalData)
{
	...
	read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
			MOT_GC13A2_VCM_BLAD_EEPROM_ADDR, MOT_GC13A2_VCM_BLAD_EEPROM_DATA_SIZE, (unsigned char *)pCamCalData->DumpAllEepromData);
	if (read_data_size <= 0) {
		return CAM_CAL_ERR_NO_DEVICE;
	}
	//mnf check
	checkSum = (pCamCalData->DumpAllEepromData[MOT_GC13A2_VCM_BLAD_MNF_CHECKSUM_ADDR])<< 8
		|(pCamCalData->DumpAllEepromData[MOT_GC13A2_VCM_BLAD_MNF_CHECKSUM_ADDR+1]);
	debug_log("checkSum  = 0x%x", checkSum);

	if(check_crc16(pCamCalData->DumpAllEepromData, MOT_GC13A2_VCM_BLAD_MNF_DATA_SIZE, checkSum)) {
	...
	memcpy(pCamCalData->serial_number, &pCamCalData->DumpAllEepromData[MOT_GC13A2_VCM_BLAD_SERIAL_NUMBER_ADDR],
		pCamCalData->serial_number_bit);

	//af check
	checkSum = (pCamCalData->DumpAllEepromData[MOT_GC13A2_VCM_BLAD_AF_CHECKSUM_ADDR])<< 8
		|(pCamCalData->DumpAllEepromData[MOT_GC13A2_VCM_BLAD_AF_CHECKSUM_ADDR+1]);
	debug_log("checkSum  = 0x%x", checkSum);

	if(check_crc16(pCamCalData->DumpAllEepromData+MOT_GC13A2_VCM_BLAD_AF_ADDR, MOT_GC13A2_VCM_BLAD_AF_DATA_SIZE, checkSum)) {
	...
	//awb check
	if((mot_check_awb_data(pCamCalData->DumpAllEepromData+MOT_GC13A2_VCM_BLAD_AWB_ADDR, MOT_GC13A2_VCM_BLAD_AWB_DATA_SIZE + 2)) == NO_ERRORS) {
	...
	//lsc check
	checkSum = (pCamCalData->DumpAllEepromData[MOT_GC13A2_VCM_BLAD_LSC_CHECKSUM_ADDR])<< 8
		|(pCamCalData->DumpAllEepromData[MOT_GC13A2_VCM_BLAD_LSC_CHECKSUM_ADDR+1]);
	debug_log("checkSum  = 0x%x", checkSum);

	if(check_crc16(pCamCalData->DumpAllEepromData+MOT_GC13A2_VCM_BLAD_LSC_ADDR, MOT_GC13A2_VCM_BLAD_LSC_DATA_SIZE, checkSum)) {
	...
	//pdaf check
	checkSum = (pCamCalData->DumpAllEepromData[MOT_GC13A2_VCM_BLAD_PDAF_CHECKSUM_ADDR])<< 8
		|(pCamCalData->DumpAllEepromData[MOT_GC13A2_VCM_BLAD_PDAF_CHECKSUM_ADDR+1]);
	checkSum1 = (pCamCalData->DumpAllEepromData[MOT_GC13A2_VCM_BLAD_PDAF_CHECKSUM_ADDR+2])<< 8
		|(pCamCalData->DumpAllEepromData[MOT_GC13A2_VCM_BLAD_PDAF_CHECKSUM_ADDR+3]);
	debug_log("checkSum  = 0x%x, checkSum1 = 0x%x", checkSum, checkSum1);

	if(check_crc16(pCamCalData->DumpAllEepromData+MOT_GC13A2_VCM_BLAD_PDAF1_ADDR, MOT_GC13A2_VCM_BLAD_PDAF_DATA1_SIZE, checkSum)
		&& check_crc16(pCamCalData->DumpAllEepromData+MOT_GC13A2_VCM_BLAD_PDAF2_ADDR, MOT_GC13A2_VCM_BLAD_PDAF_DATA2_SIZE, checkSum1)) {
	...
	//af sync data check
	checkSum = (pCamCalData->DumpAllEepromData[MOT_GC13A2_VCM_BLAD_AF_SYNC_DATA_CHECKSUM_ADDR])<< 8
		|(pCamCalData->DumpAllEepromData[MOT_GC13A2_VCM_BLAD_AF_SYNC_DATA_CHECKSUM_ADDR+1]);
	debug_log("checkSum  = 0x%x", checkSum);
	if(check_crc16(pCamCalData->DumpAllEepromData+MOT_GC13A2_VCM_BLAD_AF_SYNC_DATA_ADDR, MOT_GC13A2_VCM_BLAD_AF_SYNC_DATA_SIZE, checkSum)) {
	...
	checkSum = (pCamCalData->DumpAllEepromData[MOT_GC13A2_VCM_BLAD_MTK_NECESSARY_DATA_CHECKSUM_ADDR])<< 8
		|(pCamCalData->DumpAllEepromData[MOT_GC13A2_VCM_BLAD_MTK_NECESSARY_DATA_CHECKSUM_ADDR+1]);
	debug_log("checkSum  = 0x%x", checkSum);
	if(check_crc16(pCamCalData->DumpAllEepromData+MOT_GC13A2_VCM_BLAD_MTK_NECESSARY_DATA_ADDR, MOT_GC13A2_VCM_BLAD_MTK_NECESSARY_DATA_SIZE, checkSum)) {
